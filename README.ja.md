...
| **左クリック** | 枠ごとにリングを並べたパネルが開く（CLI ごとにまとめて表示） |
| **カーソルを乗せる** | それぞれの使用率が出る |
| **右クリック** | 今すぐ更新／アイコンに出す対象／ほかの CLI も見る（agy・grok）／Claude の取得元／ログ／Windows起動時に開始／終了 |

左クリックで開くパネル：

![利用枠パネル](docs/panel.png)

3分ごとに自動更新する。5時間枠・週次に加えて、**モデル別の週次枠**（Claude の `週次（Fable）`）、**Luna Reserve** と**手動リセットの残り回数**（Codex）、プラン名まで出る。
リングはトレイのアイコンと同じもので、CLI ごとの色・70% 超で琥珀・90% 超で赤。画面の実際の DPI で描くのでぼやけない。
